...
| hybrid_pipeline | 自研 | 待做（Reason → `run_gen_pipeline`） |
| Router | 自研 | 待做 |
| pipeline.py | 自研 | 待做 |
...
2. 写 `reason/hybrid_pipeline.py`（复用 `run_gen_pipeline`）
3. 准备 `data/input/hybrid/hybrid_cases.jsonl`（如「一束花变两束玫瑰」）
4. 验收：原图 → 推理链 → `scene_prompt` → 新图
