...
3. **假设性指令 + 需整图重生成** → **hybrid**：Reason Agent 展开场景 → GenPilot 生成。
...
**适用输入**：有参考图 + 假设性指令，但变化幅度大，**不适合指令式局部改图**（需整图重生成）。

**典型例子**：「如果把这束花分成两束玫瑰会是什么样子」「如果这张室内照变成深夜会是什么样子」——构图、物体数量或整体氛围都会变。
...
| Reason Agent（hybrid） | 自研 | 接口已有，待 `hybrid_pipeline` |
| hybrid_pipeline | 自研（B+A） | 待做 |
| Router | 自研 | 待做 |
| pipeline.py | 自研 | 待做 |
...
1. `reason/reason_agent.py` 已支持 `mode="hybrid"`：输出 `scene_prompt`（与 edit 共用 `prompts/reason_system.txt`）
2. 写 `reason/hybrid_pipeline.py`：
   ```python
   reason_result = run_reason_agent(image, instruction, mode="hybrid")
   return run_gen_pipeline(reason_result.scene_prompt, output_dir)
   ```
3. 准备 `data/input/hybrid/hybrid_cases.jsonl` + 1–2 个 hybrid case（如「一束花变两束玫瑰」）
4. 验收：原图 → 推理链 → 优化 prompt → 新图，且新图符合假设
...
- [ ] **hybrid**：1 例（如一束花变两束玫瑰），原图 vs 新图 + 推理链
